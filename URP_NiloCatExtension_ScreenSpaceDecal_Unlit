...
Shader "Universal Render Pipeline/NiloCat Extension/Screen Space Decal/Unlit"
{
    Properties
    {
        ...
        [HDR]_Color("_Color (default = white)", color) = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                //***WARNING***: "viewRay z division" must do in the fragment shader, not vertex shader! (due to rasteriazation varying interpolation's perspective correction)
            ...
            {
                //***WARNING***: now do "viewRay z division" that we skipped in vertex shader before.
                i.viewRayOS /= i.viewRayOS.w;

                float sceneCameraSpaceDepth = LinearEyeDepth(tex2Dproj(_CameraDepthTexture, i.screenUV));

				//scene depth = rayStartPos + rayDir * rayLength, all in ObjectSpace(OS)
				...
				half4 col = tex2D(_MainTex, decalSpaceUV.xy * _MainTex_ST.xy + _MainTex_ST.zw);//texture tiling+offset
